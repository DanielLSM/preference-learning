#RE-WRITE of the HumanCritic class
...
class HumanCritic(object):
    ...
    def init_tf(self):
        ...
        with self.graph.as_default():
            ...
            self.input_o0 = tf.placeholder(shape=[None, self.obs_size],
                                           dtype=tf.float32)
            self.input_o1 = tf.placeholder(shape=[None, self.obs_size],
                                           dtype=tf.float32)
            self.preference_distribution = tf.placeholder(shape=[2],
                                                          dtype=tf.float32)
            ...
            #'''
            self.model_o0_sum = tf.exp(
                tf.divide(tf.reduce_sum(self.model_o0), self.batch_sizes[0]))
            self.model_o1_sum = tf.exp(
                tf.divide(tf.reduce_sum(self.model_o1), self.batch_sizes[1]))
            #self.model_o1_sum = tf.exp(tf.reduce_sum(self.model_o1))
            self.p_o0_o1 = tf.divide(
                self.model_o0_sum, tf.add(self.model_o0_sum,
                                          self.model_o1_sum))
            self.p_o1_o0 = tf.divide(
                self.model_o1_sum, tf.add(self.model_o1_sum,
                                          self.model_o0_sum))
    ...
    def load(self, datetime_str):
        with self.graph.as_default():
            ckpt = tf.train.get_checkpoint_state("./human_critic/hc_model/" +
                                                 datetime_str)
    ...
    def train(self):
        ...
        with self.graph.as_default():
            for i in r[:batch_size]:
                ...
                with self.graph.as_default():
                    ...
                    feed_dict = {
                        self.input_o0: o0,
                        self.input_o1: o1,
                        self.preference_distribution: pref_dist,
                        self.batch_sizes: [len(o0), len(o1)]
                    }
                    m0, m1, loss, _ = self.sess.run([
                        self.model_o0_sum, self.model_o1_sum, self.loss,
                        self.train_step
                    ],
                                                    feed_dict=feed_dict)
            ...
            print("[ Loss: mean =", mean_loss, " max =", max_loss, " min =",
                  min_loss, "]")
    ...
    def add_trijactory(self, trijectory_env_name, trijectory_seed,
                       total_reward, trijectory):
        self.trijectories.append(
            [trijectory_env_name, trijectory_seed, total_reward, trijectory])
